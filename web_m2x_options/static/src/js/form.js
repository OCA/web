...
odoo.define('web_m2x_options.web_m2x_options', function (require) {
    ...
    // reference to the FieldMany2One widget.
    ...
    FieldMany2One.include({
        ...
        start: function() {
            ...
            var get_options = this.get_options();
            get_options.done(function() {
                if (_.isUndefined(self.options.no_open)) {
                    var m2x_open_undef = _.isUndefined(self.view.ir_options['web_m2x_options.open']),
                        m2x_open = self.is_option_set(self.view.ir_options['web_m2x_options.open']);
                    if (!m2x_open_undef && !m2x_open) {
                        self.options.no_open = 'True';
                    }
                }
            });
            return get_options;
