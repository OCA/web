...
Edit..." entries from many2one drop down, remove "Open External" icon button next to drop down,
You can also change default number of
proposition appearing in the drop-down. Or prevent the dialog box poping in
case of validation error.

Note, below feature has a known issue:
...
``web_m2x_options.open`` *boolean* (Default: default value is ``True``)

  Whether to display the "External button" next to dropdown panel for all fields in the odoo instance.
...
TODO: To show a scroll bar in drop down list if needed.

Below feature does not work in odoo 10, since ``widget.can_write`` always returns ``True``:
