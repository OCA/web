    ...
    const DatabaseManager = OdooClass.extend({
        ...
        getRecords: function (model_info, rc_ids) {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    let records = await this._db.all([sql]);
                    // Order by array ids
                    const res = [];
                    ...
                    return resolve(rc_ids.length === 1 ? records[0] : records);
                } catch (err) {
                    return reject(err);
                }
            });
            ...
            for (let values of datas) {
                ...
                for (let field of value_fields) {
                    ...
                    switch (model_fields[field].type) {
                        case "date":
                        {
                            values[field] = tools.SecondsToDate(values[field]);
                        } break;
                        case "datetime":
                        {
                            values[field] = tools.SecondsToDate(values[field]);
                        ...
                        case "many2many":
                        {
                            values[field] = values[field] && _.compact(values[field].split("||"));
                        } break;
                    }
