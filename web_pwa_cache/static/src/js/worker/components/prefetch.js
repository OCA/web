...
odoo.define("web_pwa_cache.PWA.components.Prefetch", function(require) {
    ...
    const SWPrefetchComponent = SWComponent.extend({
        ...
        prefetchModelInfoData: function() {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    this._sendTaskInfo(
                        "model_info_data",
                        `Getting metadata of all models, please wait...`,
                        -1,
                        0
                    );
            ...
            return new Promise(async (resolve, reject) => {
                try {
                    for (const index in model_infos) {
                        ...
                        this._sendTaskInfo(
                            "prefetch_view_data",
                            `Getting views information for the model '${model_info.model}'...`,
                            model_infos.length,
                            index
                        );
        ...
        saveModelDefaults: function(values) {
            return new Promise(async (resolve, reject) => {
                try {
                    const model_info_defaults = await this._db.getModelInfo(
                        "defaults",
                        true,
                        false,
                        true
