...
odoo.define("web.ir_actions_act_window_page", function(require) {
    ...
    ActionManager.include({
        _handleAction: function(action, options) {
            ...
            if (action.type === "ir.actions.act_window.page.list") {
                return this._executeWindowActionPageList(action, options);
            }
        ...
        _executeWindowActionPageList: function() {
            // Go back in the breadcrumb if possible, otherwise open a new list view
            if (this.controllerStack.length > 1) {
                const controller_id = this.controllerStack[
                    this.controllerStack.length - 2
                ];
                if (this.controllers[controller_id].viewType === "list") {
                    return this._restoreController(controller_id);
                }
            }
            const controller = this.getCurrentController();
            if (controller && controller.widget) {
                controller.widget.trigger_up("switch_view", {
                    view_type: "list",
                });
            }
            return Promise.resolve();
