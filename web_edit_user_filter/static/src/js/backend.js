...
odoo.define("web_edit_user_filter/static/src/js/backend.js", function (require) {
    ...
    patch(
        components.SearchBar.prototype,
        "web_edit_user_filter/static/src/js/backend.js",
        {
            mounted() {
                var self = this;
                ...
                $content.find(".list-group-item").on("click", function () {
                    ...
                    if (type === "filter") {
                        ...
                        if (FacetSelected[0].hasOptions) {
                            var OptionSelected = FacetSelected[0].options.filter(
                                function (option) {
                            ...
                            if (fetchedFilter.length) {
                                convFacets.push(
                                    self.model.get(
                                        "filters",
                                        (f) =>
                                            f.description === fc && f.type === "filter"
                                    )
                                );
                            } else {
                                ...
                                if (tentativeFilter.length) {
                                    ...
                                    self.model.dispatch("createNewFilters", [
                                        preFilter,
                                    ]);
                                }
                            }
                            ...
                            if (fetchedGroup.length) {
                                convFacets.push(
                                    self.model.get(
                                        "filters",
                                        (f) =>
                                            f.description === fc && f.type === "groupBy"
                                    )
                                );
                            } else {
                                var tentativeFilter = self.model.get(
