...
odoo.define('web_advanced_search_x2x.advanced_search', function(required) {
    ...
    this.ExtendedSearchPropositionMany2One = searchFilters.ExtendedSearchProposition.Char.extend(formCommon.FieldManagerMixin, {
	...
	get_domain : function() {
		...
		if (!this.domain || this.domain.length == 0) {
		    // TODO: the Invalid class below seems to have been removed?
		    throw new instance.web.search.Invalid(this.field.string, this.domain_representation, _t('invalid search domain'));
		}
	...
	popup_domain_selection : function() {
	    var self = this, popup = new this.SelectCreatePopup(this);
	    ...
	    popup.select_element(this.field.relation, {}, this.field.domain, new data.CompoundContext(session.user_context, this.field.context));
	},
    });

    core.search_filters_registry.add('one2many', 'web_advanced_search_x2x.ExtendedSearchPropositionMany2One');
    core.search_filters_registry.add('many2many', 'web_advanced_search_x2x.ExtendedSearchPropositionMany2One');
    core.search_filters_registry.add('many2one', 'web_advanced_search_x2x.ExtendedSearchPropositionMany2One');

    this.SelectCreatePopup = formCommon.SelectCreateDialog.extend({
	setup_search_view : function() {
		...
		self.view_list.on("list_view_loaded", self, function() {
		    ...
		    self.$buttonpane.prepend(jQuery('<button/>').addClass('oe_highlight').addClass('oe_selectcreatepopup-search-select-domain').text(_t('Use criteria')).click(
			    self.proxy(self.select_domain)));
		    self.$buttonpane.find('.oe_selectcreatepopup-search-select-domain').prop('disabled', self.searchview.build_search_data().domains.length == 0);
		    self.$buttonpane.find(".oe_selectcreatepopup-search-select").unbind('click').click(function() {
