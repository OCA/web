...
class Base(models.AbstractModel):
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if self.env.context.get("merge_notebook_tabs", True):
    ...
    def _merge_notebook_tab_update_arch(self, arch, view):
        ...
        for merge_setting in merge_settings:
            ...
            new_tab = arch.makeelement(
                ...
                string=merge_setting.tab_string,
            )
            _position = 0
            if merge_setting.tab_name_position:
                _current_index = 0
                for _page in arch.xpath("//page"):
                    _current_index += 1
                    if _page.get("name") == merge_setting.tab_name_position:
                        _position = _current_index

            notebook.insert(_position, new_tab)
