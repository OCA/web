...
Each language can be toggled between the rich-text editor and a raw
**HTML source** ("technical") view, so content the editor cannot handle
safely can still be translated. Values that contain markup the editor
cannot round-trip (full documents, ``<style>`` blocks or QWeb directives
such as those found in email templates) open in the HTML source view by
default.
