...
class SurveyQuestion(models.Model):
    ...
    @api.depends('row_ids.reference')
    def _compute_is_ref(self):
        for question in self:
            question.is_ref = any(question.row_ids.mapped('reference'))

    @api.depends('col_ids.validation_min_float_value','col_ids.validation_max_float_value','col_ids.validation_length_min','col_ids.validation_length_max')
    def _compute_is_col_max(self):
        for question in self:
            question.is_col_max = any(question.col_ids.mapped('validation_min_float_value')) or any(question.col_ids.mapped('validation_max_float_value')) or\
                                  any(question.col_ids.mapped('validation_length_min')) or any(question.col_ids.mapped('validation_length_max'))

    @api.depends(
        'row_ids.validation_min_float_value','row_ids.validation_max_float_value','row_ids.validation_length_min','row_ids.validation_length_max')
    def _compute_is_row_max(self):
        for question in self:
            question.is_row_max = any(question.row_ids.mapped('validation_min_float_value')) or any(
                question.row_ids.mapped('validation_max_float_value')) or \
                                  any(question.row_ids.mapped('validation_length_min')) or any(
                question.row_ids.mapped('validation_length_max'))
    ...
    matrix_subtype = fields.Selection([('simple', 'One choice per row'),
        ...
        ('number', 'number'),
        ('text', 'text')], string='Matrix Type', default='simple')
    ...
    row_ids = fields.One2many('question.row', 'question_id', string='Rows', copy=True)
    col_ids = fields.One2many('question.column', 'question_id', string='Columns', copy=True)
    image = fields.Binary()
    is_ref = fields.Boolean(string='Reference code in use', compute = '_compute_is_ref')
    is_col_max = fields.Boolean(string='max in use by col', compute='_compute_is_col_max')
    is_row_max = fields.Boolean(string='max in use by row', compute='_compute_is_row_max')
    ...
    def validate_matrix(self, post, answer_tag):
        ...
        if self.constr_mandatory:
            ...
            elif self.matrix_subtype in ['text', 'number']:
                pass
            else:
...
class QuestionRow(models.Model):
    """ question row definition """
    _name = 'question.row'
    _order = 'sequence,id'
    _description = 'Question Row'

    question_id = fields.Many2one('survey.question', string='Question', ondelete='cascade')
    name = fields.Char(string='Row Label')
    sequence = fields.Integer('Sequence order', default=10)
    data_type = fields.Selection([('text', 'Text'),
                                  ('number', 'Number')], string='Data Type', default='number')
    unit = fields.Char()
    validation_length_min = fields.Integer('Minimum Text Length')
    validation_length_max = fields.Integer('Maximum Text Length')
    validation_min_float_value = fields.Float('Minimum value')
    ...
    accy = fields.Float(string='decimal precision')
    reference = fields.Char('Reference', help='reference key or formula using reference key')
    reference_keys = fields.Char('Reference Keys', help='reference key extracted')
    ref_key_count = fields.Integer('Count of referred keys', help="used to track and trigger the calculation")
    quizz_mark = fields.Float('Score for this choice', help="A positive score indicates a correct choice; a negative or null score indicates a wrong answer")
    ...
    def onchange_question(self):
        if self.question_id:
            self.data_type = self.question_id.col.matrix_subtype

    @api.onchange('reference')
    def onchange_reference(self):
        if self.reference:
            reference_keys = re.split('=|\+|-|\*|\/|\(|\)', self.reference)  # split by + - * / ( )
            self.ref_key_count = len(set(filter(None, reference_keys)))  # use set to remove duplicate A1 in formula
            if reference_keys:
                self.reference_keys = ','.join(filter(None, reference_keys))   # remove empty strings in list
        else:
            self.reference_keys = ''
            self.ref_key_count = 0

class QuestionColumn(models.Model):
    """ question column """
    _name = 'question.column'
    _order = 'sequence,id'
    _description = 'Question Column'

    question_id = fields.Many2one('survey.question', string='Question', ondelete='cascade')
    name = fields.Char(string='Column Label')
    data_type = fields.Selection([('text', 'Text'),
                                ('number', 'Number')], string='Data Type', default='number')
    sequence = fields.Integer('Sequence order', default=10)
    unit = fields.Char()
    ...
    validation_error_msg = fields.Char('Validation Error message', translate=True,
                                       default=lambda self: _("The answer you entered has an invalid format."))
    accy = fields.Float(string='decimal precision')
    reference_code = fields.Char('Reference', help='reference key or formula using reference key')
    quizz_mark = fields.Float('Score for this choice', help="A positive score indicates a correct choice; a negative or null score indicates a wrong answer")
    Col01Label = fields.Char()
    Col02Label = fields.Char()
    Col03Label = fields.Char()
    Col04Label = fields.Char()
    Col01MaxNum = fields.Char()
    Col02MaxNum = fields.Char()
    Col03MaxNum = fields.Char()
    Col04MaxNum = fields.Char()
    Col01MinNum = fields.Char()
    Col02MinNum = fields.Char()
    Col03MinNum = fields.Char()
    Col04MinNum = fields.Char()
    Col01Unit = fields.Char()
    Col02Unit = fields.Char()
    Col03Unit = fields.Char()
    Col04Unit = fields.Char()
...
class QuestionListBox(models.Model):
    """ List Box """
    _name = 'question.list.box'
    _order = 'sequence,id'
    _description = 'Question Column'
    ...
    name = fields.Char(string='Column Label')
    sequence = fields.Integer('Sequence order', default=10)
    row_id = fields.Many2one('question.row', string='Row')
    column_id = fields.Many2one('question.column', string='Column')
    text = fields.Text()
...
class SurveyUserInputStatus(models.Model):
    _name = 'survey.user_input_status'
    _description = 'Survey User Input status by question'
    ...
    status = fields.Selection([('complete', 'Complete'),
        ('in_progress', 'In Progess')], string='Status')
    result = fields.Selection([('fail', 'Fail'),
        ('pass', 'Pass')], string='Result')
    fail_count = fields.Integer(string='Failed count')
    test_count = fields.Integer(string='Tested count')
    completed_by = fields.Many2one('res.users', string=' Completed By User')
    remark = fields.Text()
...
class SurveyUserInputLine(models.Model):
    ...
    answer_type = fields.Selection([
        ...
        ('image', 'Image')], string='Answer Type')
    ...
    row_id = fields.Many2one('question.row')
    col_id = fields.Many2one('question.column')
    image = fields.Binary()
            ...
            for col in question.labels_ids:
                for row in question.labels_ids_2:
                    ...
                    if a_tag in ca_dict:
                        ...
                        self.create(vals)
        elif question.matrix_subtype in ['text', 'number']:
            for col in question.col_ids:
                for row in question.row_ids:
                    a_tag = "%s_%s_%s" % (answer_tag, row.id, col.id)
                    if a_tag in ca_dict:
                        no_answers = False
                        vals.update({'answer_type': 'matrix_text', 'value_text': post[a_tag], 'row_id': row.id, 'col_id': col.id})
                        self.create(vals)
        if no_answers:
            vals.update({'answer_type': None, 'skipped': True})
            self.create(vals)
    ...
    @api.model
    def save_line_spec(self, user_input_id, question, post, answer_tag):
        vals = {
            'user_input_id': user_input_id,
            'question_id': question.id,
            'survey_id': question.survey_id.id,
    ...
    @api.model
    def save_line_pic(self, user_input_id, question, post, answer_tag):
        vals = {
            'user_input_id': user_input_id,
            'question_id': question.id,
            'survey_id': question.survey_id.id,
        ...
        if answer_tag in post and post[answer_tag]:
            file = post.get(answer_tag)
            # attach = file.stream
            # f = attach.getvalue()
            vals.update({'answer_type': 'image', 'image': base64.encodestring(file.read())})
        else:
            if "%s_%s" %s (answer_tag, 'image') in post:    # use go back to previous page
