...
odoo.define("web_widget_one2many_product_picker_sale_elaboration.AbstractView", function (require) {
    "use strict";
    ...
    AbstractView.include({
        ...
        _injectSaleElaborationFields: function (viewInfo) {
            ...
            // Add is_elaboration?
            var $field = $arch.find("field[name='is_elaboration']");
            if (!$field.length) {
                $("<FIELD/>", {
                    name: "is_elaboration",
                    invisible: 1,
            ...
            if (!$field.length) {
                $("<FIELD/>", {
                    ...
                    can_write: "true",
                    class: "mb-1",
                    attrs: "{'invisible': [['is_elaboration', '!=', True]], 'readonly': [('state', 'in', ('done', 'cancel'))}",
                    modifiers: "{\"invisible\": [[\"is_elaboration\", \"!=\", true]], \"readonly\": [[\"state\", \"in\", [\"done\", \"cancel\"]]]}",
                }).appendTo($arch);
            }
