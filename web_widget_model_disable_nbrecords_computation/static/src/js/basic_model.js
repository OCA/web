    ...
    BasicModel.include({
        _fetchSpecialDomain: function (record, fieldName, fieldInfo) {
            ...
            return self._rpc({
                    ...
                    var defaultAutoFetch = rec.length<1 ? true : rec[0].field_domain_widget_default_auto_fetch_nbrecords 
                    if (defaultAutoFetch) {
                        if (fieldInfo.options.autoFetch != undefined && !fieldInfo.options.autoFetch) {
                        ...
                        if (!defaultAutoFetch && fieldInfo.options.autoFetch != undefined && fieldInfo.options.autoFetch) {
                            return _super.apply(this, _arguments);
                        } else {
                            return Promise.resolve({
                                model: domainModel,
